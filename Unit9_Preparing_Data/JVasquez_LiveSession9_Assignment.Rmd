...
1.   Web Scrape IMDB Harry Potter and the Deathly Hallows Part II
    a.   [IMDB Full Cast List Website](https://www.imdb.com/title/tt1201607/fullcredits?ref_=tt_ql_1)
    b.   Create a data frame of the cast list
    c.   Tidy the data
        1.   No NULL rows
        2.   Two columns (Actor & Character)
        3.   Fix the white spaces in the names
        4.   Find and remove any unnecessary observations
    d.   Split the actors name into FirstName & SurName
        1.   Middle names and initials should be part of FirstName
    e.   Shows first ten rows
```{r HarryPotter_WebScrap}
library(rvest)
library(tidyr)
library(stringr)
library(ggplot2)
...
## Web Scrapping NBA Stats

1.   Web Scrape Spurs NBA Shooting Stats
    a.   [Spurs Shooting Stats Website](http://www.espn.com/nba/team/stats/_/name/sa/san-antonio-spurs)
    b.   Create a data frame of Shooting Statistics
    c.   Tidy the data
        1.   No NULL rows or the total row
        2.   Split player name into name and position
        3.   Convert the data to numeric values & check
    d.   Create bar plot of 'Field Goals Per Game' by player
        1.   Informative title, centered
        2.   Label X & Y axis informatively
        3.   Make the graph a horizontal bar plot
        4.   Color the plot by positions
